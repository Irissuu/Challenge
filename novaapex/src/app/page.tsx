  ...
  const handleAuth = async () => {
    ...
    try {
      if (isLogin) {
        // Login
        if (email && senha) {
          const response = await fetch("http://localhost:8080/challenge1_war/api/usuarios/login", {
            method: "POST",
            headers: {
        ...
        if (nome && email && cpf && contato && senha) {
          const response = await fetch("http://localhost:8080/challenge1_war/api/usuarios/cadastro", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
